...
OPP-078A	NEMAR Research Infrastructure	Establishing the reproducible NEMAR EEG-data access workflow inside the private Neuro-TMR engineering environment	Direct technical execution	2026-08-25	IN_PROGRESS	Verify NEMAR tooling in WSL, identify the initial sleep dataset, retrieve metadata, then selectively retrieve a small usable data subset	Immediate	First active Pre-Path V technical execution. Engineering work is performed only in the private engineering repo; A6 records status and high-level outcome only.
...
Pre-Path V Execution Snapshot — 2026-08-25
Opportunity	Current State	Immediate Requirement
OPP-078A — NEMAR	IN_PROGRESS	Establish reproducible EEG/PSG data-access workflow
...
Environment → dataset discovery → metadata → small subset → ingestion verification → reproducibility

This is the current active execution target.
